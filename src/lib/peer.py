...
class Peer(object):
    ...
    def handle_have(self, have):
        ...
        if self.is_choked() and not self.state['am_interested']:
    ...
    def _handle_keep_alive(self):
        ...
        except message.WongMessageException:
            return False
        except Exception as e:
            logging.exception("Error KeepAlive, need at last 4 bytes: {}\n"
                              "{}".format(len(self.read_buffer), e.__str__()))
            return False
    ...
    def get_message(self):
        while len(self.read_buffer) > 4 and self.healthy:
            if (not self.has_handshake and self._handle_handshake()) or self._handle_keep_alive():
                continue
            ...
            try:
                receive_message = message.MessageDispatcher(payload).dispatch()
                if receive_message:
                    yield receive_message
            except message.WongMessageException as e:
